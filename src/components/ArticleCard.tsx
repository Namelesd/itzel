...
function formatDate(date: Date): string {
  return new Intl.DateTimeFormat('es-MX', { day: 'numeric', month: 'long', year: 'numeric' }).format(new Date(date))
}
  ...
  return (
    <article style={{ border: '0.5px solid #e0ddd6', borderRadius: '12px', padding: '1rem 1.25rem', background: '#fff', marginBottom: '10px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px', flexWrap: 'wrap' }}>
        ...
        <span style={{ fontSize: '10px', fontWeight: 500, padding: '2px 8px', borderRadius: '20px', background: category.bg, color: category.color }}>{category.label}</span>
        {article.municipality && (
          <span style={{ fontSize: '11px', color: '#888780' }}>• {article.municipality}{article.state ? `, ${article.state}` : ''}</span>
        )}
        <span style={{ fontSize: '11px', color: '#888780', marginLeft: 'auto' }}>{formatDate(article.publishedAt)}</span>
      </div>

      <a href={article.url} target="_blank" rel="noopener noreferrer">
        <div style={{ fontSize: '15px', fontWeight: 500, color: '#1a1a1e', lineHeight: '1.4', marginBottom: '6px' }}>{article.title}</div>
      </a>

      {article.excerpt && (
        <p style={{ fontSize: '13px', color: '#5f5e5a', lineHeight: '1.6', marginBottom: '10px' }}>{article.excerpt.slice(0, 200)}</p>
      )}
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', paddingTop: '10px', borderTop: '0.5px solid #f0ede6', flexWrap: 'wrap', gap: '8px' }}>
        ...
        {journalist ? (
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
            ...
            <a href={'/journalist/' + journalist.slug} style={{ fontSize: '12px', fontWeight: 500, color: '#1a1a1e', textDecoration: 'none' }}>{journalist.name}</a>
           <a href={'/journalist/' + journalist.slug + '#article-' + article.id} title="Ver desglose de fidelidad" style={{ fontSize: '11px', fontWeight: 500, padding: '2px 10px', borderRadius: '20px', background: getFidelityBg(journalist.fidelity), color: getFidelityColor(journalist.fidelity), textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '4px' }}>
  <span style={{ width: '6px', height: '6px', borderRadius: '50%', background: getFidelityColor(journalist.fidelity), display: 'inline-block' }} />
  {'Fidelidad ' + Math.round(journalist.fidelity) + '% · ' + getFidelityLabel(journalist.fidelity)}
</a>
          </div>
        ) : (
        ...
        {journalist && (
          <a href={'/journalist/' + journalist.slug} style={{ fontSize: '11px', color: '#888780', textDecoration: 'none', border: '0.5px solid #d3d1c7', borderRadius: '20px', padding: '3px 10px' }}>Ver periodista →</a>
        )}
