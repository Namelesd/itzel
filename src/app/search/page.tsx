    ...
    <div style={{ maxWidth: '860px', margin: '0 auto', padding: '2rem 1.5rem', minHeight: '100vh', background: '#f5f0e8' }}>
      ...
      <form action="/search" method="GET" style={{ marginBottom: '1.5rem' }}>
        <div style={{ display: 'flex', gap: '10px', marginBottom: '1rem' }}>
          ...
          <button
            type="submit"
            style={{ padding: '10px 24px', background: '#1a1a1e', color: '#f5f0e8', border: 'none', borderRadius: '24px', fontSize: '13px', cursor: 'pointer', fontFamily: 'inherit' }}
          >
            Buscar
          </button>
        </div>
        ...
        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
          ...
          <a href={baseUrl + (query ? '?query=' + query : '')} style={{ fontSize: '12px', padding: '4px 12px', borderRadius: '20px', border: '0.5px solid #d3d1c7', textDecoration: 'none', background: !category ? '#1a1a1e' : '#fff', color: !category ? '#f5f0e8' : '#5f5e5a' }}>Todas</a>

          {categories.map(cat => (
            <a key={cat} href={baseUrl + '?category=' + cat + (query ? '&query=' + query : '')} style={{ fontSize: '12px', padding: '4px 12px', borderRadius: '20px', border: '0.5px solid #d3d1c7', textDecoration: 'none', background: category === cat ? '#1a1a1e' : '#fff', color: category === cat ? '#f5f0e8' : '#5f5e5a' }}>{LABELS[cat] ?? cat}</a>
          ))}
      ...
      {results.pages > 1 && (
        <div style={{ display: 'flex', justifyContent: 'center', gap: '8px', marginTop: '2rem' }}>
          {Array.from({ length: results.pages }, (_, i) => i + 1).map(p => (
            <a key={p} href={baseUrl + '?query=' + query + (category ? '&category=' + category : '') + '&page=' + p} style={{ width: '36px', height: '36px', display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: '50%', border: '0.5px solid #d3d1c7', textDecoration: 'none', fontSize: '13px', background: p === page ? '#1a1a1e' : '#fff', color: p === page ? '#f5f0e8' : '#5f5e5a' }}>{p}</a>
          ))}
        </div>
      )}
